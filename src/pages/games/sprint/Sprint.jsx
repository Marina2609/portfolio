...
const Sprint = () => {
  ...
  const [questionWordTranslate, setQuestionWordTranslate] = useState("");
  ...
  const fetchPartialWords = useCallback(async () => {
    ...
    if (group && page) {
      wordsPartial = await Service.getWords(+group - 1, +page - 1);
    } else {
      wordsPartial = await Service.getWords(currentGroup - 1, currentPage);
    }
  ...
  const createCorrectWord = useCallback(
    async (wordId) => {
      if (isAuth) {
        ...
        const totalCorrectAnswersSprintGame =
          optional.totalCorrectAnswersSprintGame + 1;
