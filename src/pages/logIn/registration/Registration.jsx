...
import Service, {
  DataUserCreateResponse,
  DataUserLoginResponse,
...
const Registration = () => {
  ...
  const closeFormHandler = (event) => {
    if (event.target.classList?.contains("page-registration")) {
      navigate("/");
    }
  };
  ...
  const submitHandler = async (event) => {
    ...
    const responseRegistration = await Service.createUser({
      name,
      email,
      password,
    });
    ...
    if (!responseRegistration) {
      setSuccess(false);
      setRegistrationStatusMessage(
        `Пользователь с ${email} уже зарегистрирован!`
      );
    } else {
      setSuccess(true);
  ...
  return (
    <div
      className="page-registration wrapper-form"
